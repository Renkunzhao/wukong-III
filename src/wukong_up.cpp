...
int main(int argc, char* argv[]) {
  ...
  vector<double> q1,q2,q3,dq1,dq2,dq3;
  vector<double> pltt,pltz,pltzd,plttheta,pltthetad,a1,a2,a3,a4,a5,a6,a7,a8,a9,f1,f2,f3,fJac1,fJac2,fJac3,fAda1,fAda2,fAda3;
  //增加负载
  for (int i=0; i<10000; i++) {
    ...
    if(i%100==0){
      ...
      pltt.push_back(time);
      pltzd.push_back(CoMz+0.045);
      pltthetad.push_back(CoMTheta);
      q1d.push_back(posTarget(27));
      q2d.push_back(posTarget(25));
      q3d.push_back(posTarget(24));
      dq1d.push_back(velTarget(26));
      dq2d.push_back(velTarget(24));
      dq3d.push_back(velTarget(23));      
      ...
      q2.push_back(pos(25));
      q3.push_back(pos(24));
      dq1.push_back(vel(26));
      dq2.push_back(vel(24));
      dq3.push_back(vel(23));
    ...
    // auto cyclet = fmod(time-2,4);
    // if(cyclet>0&&cyclet<1){
    //   CoMz = 0.5;
    //   endVr << 0,0,0,0,0,0;
    //   endVl << 0,0,0,0,0,0;      
    // }
    // else if(cyclet>1&&cyclet<2){
    //   CoMz = 0.5 + 0.2*(cyclet-1);
    //   endVr << 0,0,-0.2,0,0,0;
    //   endVl << 0,0,-0.2,0,0,0;
    // }
    // else if(cyclet>2&&cyclet<3){
    //   CoMz = 0.7;
    //   endVr << 0,0,0,0,0,0;
    //   endVl << 0,0,0,0,0,0;
    // }
    // if(cyclet>3&&cyclet<4){
    //   CoMz = 0.7 - 0.2*(cyclet-3);
    //   endVr << 0,0,0.2,0,0,0;
    //   endVl << 0,0,0.2,0,0,0;
    // }
    CoMz = 0.6 + 0.1*sin(0.5*pi*(time-3));
    endVr << 0,0,-0.1*0.5*pi*cos(0.5*pi*(time-3)),0,0,0;
    endVl << 0,0,-0.1*0.5*pi*cos(0.5*pi*(time-3)),0,0,0;
    ...
    if(i%20==0){
      ...
      // vel
      velTarget.block(15,0,6,1) = wkKin.RJac.fullPivLu().solve(endVr);
      velTarget.block(21,0,6,1) = wkKin.LJac.fullPivLu().solve(endVl);
      wk3->setPdTarget(posTarget, velTarget);
    }
  ...
  plt::figure(6);
  plt::plot(pltt, dq3, "m-", {{"label", "HipY"}});
  plt::plot(pltt, dq3d, "k:", {{"label", "HipY$_{set}$"}});
  plt::legend();
  plt::xlabel("时间(s)");
  ...
  plt::figure(7);
  plt::plot(pltt, dq2, "g-", {{"label", "KneeY"}});
  plt::plot(pltt, dq2d, "k:", {{"label", "KneeY$_{set}$"}});
  plt::legend();
  plt::xlabel("时间(s)");
  plt::ylabel("角速度(rad/s)");
  ...
  plt::figure(8);
  plt::plot(pltt, dq1, "r-", {{"label", "AnkleY"}});
  plt::plot(pltt, dq1d, "k:", {{"label", "AnkleY$_{set}$"}});
  plt::legend();
  plt::xlabel("时间(s)");
  plt::ylabel("角速度(rad/s)");
